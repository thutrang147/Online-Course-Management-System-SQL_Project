        ...
        try:
            if choice == 0: # List All
                ...
                ui.display_table(["LearnerID", "LearnerName", "Email", "PhoneNumber"], 
                                 learners, 
                                 "No learners found.")
            elif choice == 1: # Add New Learner
                name = ui.get_input("Enter learner name", str)
                email = ui.get_input("Enter learner email", str)
                if not is_email_unique(email):
                    ui.display_message("Email already exists for another user. Please use a different email.", "error")
                    continue
                password = ui.get_input("Enter password for learner", str)
                phone = ui.get_input("Enter learner phone (optional)", str, required=False)
                # Sửa lại thứ tự: name, phone, password
                learner_id = learner_manager.add_learner(name, email, phone if phone else None, password)
...
def admin_handle_instructor_management():
    ...
    while True:
        ...
        try:
            if choice == 0: # List All
            ...
            elif choice == 1: # Add New Instructor
                ...
                if not is_email_unique(email):
                    ui.display_message("Email already exists for another user. Please use a different email.", "error")
                    continue
                password = ui.get_input("Enter password for instructor", str)  # Thêm dòng này
                instructor_id = instructor_manager.add_instructor(name, expertise if expertise else None, email, password)  # Thêm password
...
def handle_learner_portal(learner_id_portal):
    """Hiển thị và xử lý portal của learner."""
    selected_learner = learner_manager.get_learner_by_id(learner_id_portal)
    if not selected_learner:
        ui.display_message(f"Learner with ID {learner_id_portal} not found.", "error")
        ui.pause_screen()
        return
    ...
    # Tiếp tục với menu và các chức năng...
    # ...existing code...
    while True:
        ...
        try:
            if choice == 0: # View My Enrolled Courses
                ...
                userId = learner_manager.get_user_id_by_learner_id(learner_id_portal)
                if not userId:
                    ui.display_message("User ID not found for this learner.", "error")
                    continue
                handle_change_password(userId)
            elif choice == 4: # Back
                break
            if choice != 4: ui.pause_screen()
        except Exception as e:
...
def handle_instructor_portal(instructor_id_portal):
    """Hiển thị và xử lý portal của instructor."""
    selected_instructor = instructor_manager.get_instructor_by_id(instructor_id_portal)
    if not selected_instructor:
        ui.display_message(f"Instructor with ID {instructor_id_portal} not found.", "error")
        ui.pause_screen()
        return
    ...
    portal_menu_options = [
        ...
        "Manage Lectures for an Assigned Course",
        "View Enrollment Roster & Progress for My Courses", 
        "Change Password",
        "Back to Main Menu"
    ]
    ...
    while True:
        ...
        try:
            if choice == 0: # My Assigned Courses
                ...
                userID = instructor_manager.get_user_id_by_instructor_id(instructor_id_portal) 
                handle_change_password(userID)
            elif choice == 4: # Back
                break
            if choice != 4: ui.pause_screen()
...
def is_email_unique(email):
    from managers.user_manager import get_user_by_email
    return get_user_by_email(email) is None

def handle_change_password(user_id):
    """Cho phép user đổi mật khẩu."""
    from managers import user_manager
    
    current_password = ui.get_input("Enter current password", str)
    user = user_manager.get_user_by_id(user_id)
    
    if not user or not bcrypt.checkpw(current_password.encode('utf-8'), user['Password'].encode('utf-8')):
        ui.display_message("Current password is incorrect", "error")
        return False
        
    new_password = ui.get_input("Enter new password", str)
    confirm_password = ui.get_input("Confirm new password", str)
    
    if new_password != confirm_password:
        ui.display_message("Passwords do not match", "error")
        return False
    
    if user_manager.update_password(user_id, new_password):
        ui.display_message("Password changed successfully", "success")
        return True
    else:
        ui.display_message("Failed to change password", "error")
        return False

def handle_learner_registration():
    ui.display_message("=== Learner Registration ===")
    name = ui.get_input("Enter your full name", str)
    email = ui.get_input("Enter your email", str)
    from managers.user_manager import get_user_by_email
    if not is_email_unique(email):
        ui.display_message("Email already exists. Please use a different email.", "error")
        return
    password = ui.get_input("Enter your password", str)
    phone = ui.get_input("Enter your phone (optional)", str, required=False)
    # Thêm learner mới
    from managers.learner_manager import add_learner
    learner_id = add_learner(name, email, phone if phone else None, password)
    if learner_id:
        ui.display_message("Registration successful! You can now log in.", "success")
    else:
        ui.display_message("Registration failed. Please try again.", "error")

# --- Main Application Loop (Role-Based) ---
def main():
    while True:
        ui.display_message("Welcome! Please choose an option.")
        choice = ui.get_input("1. Login\n2. Register as Learner\n3. Exit\nEnter your choice", int, allowed_values=['1', '2', '3'])
        if choice == 1:
            # Đăng nhập như cũ
            email = ui.get_input("Email", str)
            password = ui.get_input("Password", str)
            user = auth_manager.authenticate(email, password)
            if not user:
                ui.display_message("Invalid credentials. Try again.", "error")
                continue
            role = user['role']
            if role == 'learner':
                handle_learner_portal(user['id'])
            elif role == 'instructor':
                handle_instructor_portal(user['id'])
            elif role == 'admin':
                handle_administrator_panel()
            else:
                ui.display_message("Unknown role.", "error")
        elif choice == 2:
            handle_learner_registration()
        elif choice == 3:
            break
